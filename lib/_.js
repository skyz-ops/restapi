...
(exports.Primbon = Primbon),
  (exports.musicaldown2 = async (a) =>
    new Promise((t, n) => {
      axios
        ...
        .then((n) => {
          const i = cheerio.load(n.data),
            e = i("#link_url").attr("name"),
            s = i("#submit-form > div")
            ...
            l = i("#submit-form > div")
              ...
              .attr("value");
          let o = { [`${e}`]: a, [`${s}`]: r, verify: l };
          axios
            .request({
              url: "https://musicaldown.com/id/download",
              method: "post",
              data: new URLSearchParams(Object.entries(o)),
              headers: {
                "user-agent":
                  "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.69 Safari/537.36",
                cookie: n.headers["set-cookie"],
              },
            })
            .then((a) => {
              const i = cheerio.load(a.data);
              axios
                .request({
                ...
                .then((a) => {
                  const n = cheerio.load(a.data),
                    e = {
                      creator: "Arya-kun >///<",
                      video: i("body > div.welcome.section > div")
                        .find(
                          "div:nth-child(2) > div.col.s12.l8 > a:nth-child(4)"
                        )
                        .attr("href"),
                      audio: n(
      ...
      request(
        {
          ...
          headers: {
            ...
            cookie:
              "_ga=GA1.2.445492804.1636121016; _gid=GA1.2.592519710.1636121016; __gads=ID=9187eb4307c758af-225ad2ec8dce0086:T=1636121013:RT=1636121013:S=ALNI_MZbpH62int4Hm8h3x4VIKlHek_Ndw",
          },
          formData: { data: a },
        },
        async function (a, n, i) {
          ...
          (res = JSON.parse(i)),
            ...
            t({
              ...
              cc: res.msg.includes("Live")
                ? res.msg.split("b> |")[1].split("<")[0].split("[BIN")[0]
                : res.msg.split("b> |")[1].split("<")[0],
            });
        }
      );
    })),
  (exports.konachan = (a) =>
    new Promise((t, n) => {
      let i = a.replace(" ", "_");
      axios
        .get("https://konachan.net/post?tags=" + i + "+")
        .then(({ data: a }) => {
          const n = cheerio.load(a),
            e = [];
          n("div.pagination > a").each(function (a, t) {
            e.push(n(t).text());
          });
          let s = Math.floor(Math.random() * e.length);
          axios
            .get("https://konachan.net/post?page=" + s + "&tags=" + i + "+")
            .then(({ data: a }) => {
              const n = cheerio.load(a),
                i = [];
              n(
  ...
  (exports.wattpad = (a) =>
    new Promise((t, n) => {
      axios
        .get("https://www.wattpad.com/search/" + a)
        .then(({ data: a }) => {
          const n = cheerio.load(a),
            i = [],
            e = [],
  ...
  (exports.zerochan = (a) =>
    new Promise((t, n) => {
      axios
        .get("https://www.zerochan.net/search?q=" + a)
        .then(({ data: a }) => {
          const n = cheerio.load(a),
            i = [],
            e = [],
            s = [];
          n("#thumbs2 > li > a > img").each(function (a, t) {
            n(t).attr("alt").startsWith("https://static.zerochan.net/") ||
              i.push(n(t).attr("alt"));
          }),
            n("#thumbs2 > li > a").each(function (a, t) {
              s.push(n(t).attr("href"));
            });
          ...
          t({ creator: "Fajar Ihsana", result: e });
        })
        .catch(n);
    }));
