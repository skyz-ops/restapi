const { limitAdd, isLimit, apikeyCheck } = require("../database/db");
const { download_Url } = require("../lib/functions");
...
async function photoOxy(req, res) {
  ...
  if (tema === "shadow") {
    ...
    url.push(
      "https://photooxy.com/logo-and-text-effects/creating-text-underwater-ocean-363.html"
    );
  } else if (tema === "paper") {
    url.push(
    ...
    return res.send({
      status: 404,
      message:
        "Tema Tersedia Sebagai Berikut: (The following themes are available:)",
      result: listTema,
    });
  }
  oxy(text, url[0])
